...
{
    public class MatchPlayHandicapScorer : IScoringCalculator
    {
        public PointsAwarded[] CalculatePoints(List<GolferHoleScore> scoreData)
        {
            ...
            foreach (var hole in groupedData)       // For each hole
            {
                ...
                {
                    if (score.NetScore < lowestScore || lowestScore == -1)
                    {
                        // Clear low golfer list
                        golferIds.Clear();

                        lowestScore = score.NetScore;
                        golferIds.Add(score.GolferId);
                    }
                    else if (score.NetScore == lowestScore)    // Tie on hole
                    {
